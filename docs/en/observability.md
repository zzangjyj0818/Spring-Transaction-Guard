...
| `transaction.guard.violation.total` | Counter | Once per evaluated violation | `code`: `TG001`, `TG002`, `TG003` |
...
Transaction IDs, entry methods, hosts, paths, and exception types are not metric tags. Request queries, headers, and bodies are not collected. The number of time series therefore does not grow with user input or destination count.
...
- TG001–TG003 violation counts
- HTTP summaries grouped by RestClient/OpenFeign and success/failure
- Effective duration thresholds and violation mode
